...
def test_unique_id_can_prevent_duplicate_rows(connection, geocoded_data, property_data):
    ...
    pd_full = standardise_data(property_data, address_column="address")
    #guarentees that the unique id isn't geocoded, because that's the latest point
...
def test_only_select_rows_that_have_not_been_geocoded(property_data, geocoded_data):
    property_data = standardise_data(property_data, address_column="address")
    geocoded_data = standardise_data(geocoded_data, address_column="address")
    both = property_data.merge(geocoded_data, on="unique_id", how="left")
    # breakpoint()
    assert both.shape[0] == property_data.shape[0]
    assert both.shape[0] > geocoded_data.shape[0]
    pass
