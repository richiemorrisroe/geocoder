...
def load_data_into_table(con, table_name, data: pd.DataFrame, if_exists=None):
    ...
    data.to_sql(name=table_name, con=con, if_exists=if_exists, index=False)
    return 1
...
def check_for_new_rows(connection, left_table="property_sales_stg", right_table="property_sales_geocoded", limit=None, county_name=None, from_date=None):
    if limit:
    ...
    sql = """with core as (SELECT stg.*, stg.address as original_address
    FROM {left_table} stg
    LEFT JOIN {right_table} gc
    ON
    ...
    from
    non_geocodeable_addresses
    ), final as (
    select core.* from core LEFT JOIN skip on core.original_address = skip.address AND to_skip IS NULL {limit})
