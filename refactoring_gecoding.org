...
name [fn:4]. 
# These have a number associated with them, which
...
name stuff. In general, making changes like this is dangerous, so you
want to take a bunch of small steps and add tests, to avoid causing
...
name and then we rerun the process that generates the script.
...
The way python handles files bugs me normally [fn:11], so we'll
...
#+begin_src python
  ...
  import logging
  import time
  from geocode.geocode_funcs import create_logger, get_api_key, get_google_results, add_ireland_to_address
  ...
  addresses = add_ireland_to_address(data, address_column_name)
...
It turns out I need to actually join my original data to the geocoded data,
and the best place to do that is here.

Additionally, we want to add hashing so that we don't repeat work. 

#+begin_src python 
addresses = (data[address_column_name] + ',' + data['county'] + ',Ireland').tolist()
#+end_src

This is logic from geocoder script, which adds county and country to
the input addresses (which may or may not have this already). 

We can turn it into a function, so that we can call it more easily
on other data (and test it). 

#+begin_src python :tangle geocode/join.py
...
#+BEGIN_SRC python :tangle geocoder_new.py
  ...
  # Write the full results to csv using the pandas library.
  results_df = pd.DataFrame(results)
  results_df_joined = join_input_and_output(data_pp, results_df)
  results_df_joined.to_csv(output_filename, encoding='utf8')
...
[fn:11] the with dance, though explicit, I find incredibly tedious 
